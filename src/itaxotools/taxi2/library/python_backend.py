...
def make_distance_table(table: pd.DataFrame, already_aligned: bool) -> pd.DataFrame:
    ...
    distance_table = (
        pd.DataFrame(distance_array, index=seqid1, columns=seqid2)
        .stack()
        .reset_index(name="distances")
    )
...
def make_distance_table2(
    ...
    distance_table.rename(
        columns={col: (col + " (query 2)") for col in reference_table.columns},
        inplace=True,
    )
