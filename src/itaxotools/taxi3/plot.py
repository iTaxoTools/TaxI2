...
class HistogramPlotter:
    ...
    def plot(self, output_path: Path):
        ...
        for metric in self.metrics:
            ...
            self.plot_layered(metric, df, palette, order, path / f'{metric}_layered_hist')
            self.plot_histogram(metric, df, 'stack', palette, order, path / f'{metric}_stacked_hist')
            self.plot_histogram(metric, df, 'dodge', palette, order, path / f'{metric}_dodge_hist')

            has_species_info = ComparisonType.IntraSpecies.label in types or ComparisonType.InterSpecies.label in types
            has_genus_info = ComparisonType.IntraGenus.label in types or ComparisonType.InterGenus.label in types
            ...
            if has_species_info and has_genus_info:
                ...
                for old, new in translation.items():
                    df_species['type'] = df_species['type'].str.replace(old.label, new.label, regex=False)
                ...
                self.plot_layered(metric, df_species, palette, order, path_species / f'{metric}_layered_hist_species_only')
                self.plot_histogram(metric, df_species, 'stack', palette, order, path_species / f'{metric}_stacked_hist_species_only')
                self.plot_histogram(metric, df_species, 'dodge', palette, order, path_species / f'{metric}_dodge_hist_species_only')
                ...
                df_genus = df.copy()
                for old, new in translation.items():
                    df_genus['type'] = df_genus['type'].str.replace(old.label, new.label, regex=False)
                ...
                self.plot_layered(metric, df_genus, palette, order, path_genus / f'{metric}_layered_hist_genus_only')
                self.plot_histogram(metric, df_genus, 'stack', palette, order, path_genus / f'{metric}_stacked_hist_genus_only')
                self.plot_histogram(metric, df_genus, 'dodge', palette, order, path_genus / f'{metric}_dodge_hist_genus_only')

    def plot_layered(self, metric: str, df: pd.DataFrame, palette: list[tuple], order: list[str], path: Path):
        g = sns.FacetGrid(df, row='type', hue='type', palette=palette, hue_order=order, height=1.5, aspect=4)
    ...
    def plot_histogram(self, metric: str, df: pd.DataFrame, multiple: str, palette: list[tuple], order: list[str], path: Path):
        fig, ax = plt.subplots()
        sns.histplot(df, x='value', hue='type', multiple=multiple, binwidth=self.binwidth * self.binfactor, binrange=(0.0, self.binfactor), palette=palette, hue_order=order, ax=ax)
        sns.despine()
